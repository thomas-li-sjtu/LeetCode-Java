...
// removable 中的元素 互不相同 
// 
// Related Topics 数组 字符串 二分查找 👍 37 👎 0
...
class Solution1898 {
    public int maximumRemovals(String s, String p, int[] removable) {
        ...
        if (left == removable.length) {
            left = removable.length-1;
        }
    ...
    public boolean check(boolean[] remove, String s, String p) {
        int point1 = 0;
        int point2 = 0;
        while (point1 < s.length() && point2 < p.length()) {
            if (!remove[point1] && s.charAt(point1) == p.charAt(point2)) {
                point1 += 1;
                point2 += 1;
            } else if (remove[point1] && s.charAt(point1) == p.charAt(point2)) {
                point1 += 1;
            } else if (s.charAt(point1) != p.charAt(point2)) {
                point1 += 1;
            } else {
