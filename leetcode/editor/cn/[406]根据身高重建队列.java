...
// 题目数据确保队列可以被重建 
// 
// Related Topics 贪心 树状数组 线段树 数组 排序 👍 1392 👎 0
...
class Solution406 {
    public int[][] reconstructQueue(int[][] people) {
        // 遇到两个维度权衡的时候，一定要先确定一个维度，再确定另一个维度，而且一个正序一个反序
        // 身高从大到小排（身高相同k小的站前面）
        ...
        for (int[] p : people) {  // 从 A 里挑身高最高的人 x 出来，插入到 B. 因为 B 中每个人的身高都比 x 要高，因此 x 插入的位置就是看 x 前面应该有多少人就行了
            que.add(p[1],p);
        }
